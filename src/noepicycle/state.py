...
class LoopState(TypedDict):
    ...
    tokens_spent: int
    current_model: str
    visited_models: Annotated[list[str], operator.add]
    grace_period_remaining: int
    solution_hashes: Annotated[list[str], operator.add]
    fixation_count: int
...
def build_debug_thread(history: list[IterationRecord]) -> str:
    ...
    for record in history:
        ...
        if record["solution_hash"] in repeated_hashes:
            ...
            if count > 1:
                fixation_flag = f"  ⚠ SAME SOLUTION AS ITERATION {hashes.index(record['solution_hash']) + 1}"
    ...
    if repeated_hashes and len(history) > 1:
        lines.append(
            ...
            "multiple times. Try a fundamentally different algorithm or data structure."
        )
